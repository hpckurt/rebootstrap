...
patch_newt() {
	echo "fix nopython profile #1082963"
	drop_privs patch -p1 <<'EOF'
--- a/Makefile.in
+++ b/Makefile.in
@@ -13,8 +13,13 @@
...
+CONFIGURE_ARGS += --with-nostrip=$(if $(filter notsrip,$(DEB_BUILD_OPTIONS)),yes,no)
+
...
+	dh_auto_configure  -- $(CONFIGURE_ARGS) CFLAGS='-I/usr/include/tcl8.6 $(CFLAGS) -DMARCH=\"$(DEB_HOST_MULTIARCH)\" '  \
