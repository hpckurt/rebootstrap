...
DROP_PRIVS=buildd
GCC_NOLANG="ada algol68 asan brig cobol d gcn go itm java jit hppa64 lsan m2 nvptx objc obj-c++ rust tsan ubsan"
...
patch_e2fsprogs() {
	echo "fix FTBFS with updated pkgconf #1126636"
	drop_privs sed -i -e 's/pkg-config pkg-config/pkg-config/' debian/rules
}
...
--- a/include/malloc.h
+++ b/include/malloc.h
@@ -18,6 +18,22 @@ void *memalign(size_t, size_t);
...
builddep_ncurses() {
	...
	apt_get_install debhelper "pkgconf:$1" autoconf-dickey
